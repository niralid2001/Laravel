                    ...
                    // print_r($name);
                     if($name){
                         if($name[0]['admin_id']==1 && $name[0]['super_admin'] == 1)
                         {
                             ...
                             $added_by = $name[0]['name']."  (Branch-Admin)";
                         }
            ...
            // 		$cond = "students.admin_id = $admin_id AND sudent_batchs.batch_id in ($batch_ids) AND students.status = 1";
                ...
                if(!empty($student_dat)){
                    ...
                    foreach($new as $student){
                        ...
                        $join = array('batches',"batches.id = sudent_batchs.batch_id");
                          $student_batch = $this->db_model->select_data('sudent_batchs.student_id,sudent_batchs.batch_id,batches.batch_name,batches.id,batches.end_date','sudent_batchs',array('student_id'=>$student['s_id']),'','','',$join,'batches.id');
                        }
						...
						//echo "hii...."; echo $student['batch_id']; print_r($student_batch[0]['end_date']);exit;
						//updated status code
						...
						if($student['batch_id'] == 0){
							$statusDrop = '<div class="admin_tbl_status_wrap">
							<a class="tbl_status_btn light_red_bg" data-id="'.$student['student_id'].'" data-table ="students" data-status ="1" href="javascript:;">'.$this->lang->line('ltr_inactive').'</a></div>';
                        }else if($student['batch_id'] != 0 && $student_batch[0]['end_date'] > $current_date){
							$statusDrop = '<div class="admin_tbl_status_wrap"><a class="tbl_status_btn light_sky_bg" data-id="'.$student['student_id'].'" data-table ="students" data-status ="0" href="javascript:;">'.$this->lang->line('ltr_active').'</a></div>';
						}else{
							$statusDrop = '<div class="admin_tbl_status_wrap">
							<a class="tbl_status_btn btn btn-Warning" data-id="'.$student['student_id'].'" data-table ="students" data-status ="1" href="javascript:;">'.$this->lang->line('ltr_graduate').'</a></div>'; 
						}
                ...
                //     FROM (SELECT `sudent_batchs`.`id` FROM $table LEFT JOIN `students` ON `students`.`id`=`sudent_batchs`.`student_id` WHERE $cond $where_in ".($like1 != ''?"AND name LIKE '%".$like1."%' ESCAPE '!'":'')." GROUP BY `students`.`id`) sada")[0]['numrows'];
            ...
            if(!empty($this->input->post('email',TRUE))){
                ...
                //   $del = $this->db_model->delete_data('sudent_batchs',array('student_id'=>$st_id));
                    ...
                    foreach($batch_id as $value){
                     ...
                     $em_msg = $this->lang->line('ltr_hey').' '.ucwords($this->input->post('name',TRUE)).', '.$this->lang->line('ltr_congratulations').' <br/><br/>'.$this->lang->line('ltr_successfully_enrolled_in').' '.$batchName.'<br/><br/>'.$this->lang->line('ltr_enjoy').'<br/><br/> '.$this->lang->line('ltr_thanks_msg').'<br/><br/>'.$this->lang->line('ltr_bizacademy');
...
function category_table(){
    ...
    function lead_management_table(){
		//print_r("dhfkjsdf");exit;
        if(isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')){
            $post = $this->input->post(NULL,TRUE);
            $get = $this->input->get(NULL,TRUE);
			//print_r($get);
			...
			if($post['search']['value'] != ''){
                $like = array("student_lead_management.name", "'%".$post['search']['value']."%'");
                //$search="(name LIKE '%".$post['search']['value']."%')";
            }
			
			
			$this->db->select('student_lead_management.*,name ,contact');
			$this->db->from('student_lead_management use index (id)');
            $this->db->like('name',$post['search']['value'],false);
			...
			//$this->db->group_start(); 
			if($get['s_source'] != ''){
				$this->db->like('source' , $get['s_source']);
            }
			
			if($get['s_fromdate'] != ''){
			$s_fromdate = date('Y-m-d', strtotime($get['s_fromdate']));
               $this->db->or_like('enquiry_fromdate', $s_fromdate);
            }
			
			if($get['s_todate'] != ''){
			$s_todate = date('Y-m-d', strtotime($get['s_todate']));
                $this->db->or_like('enquiry_todate',  $s_todate);
            }
			
			if($get['s_status'] != ''){
                $this->db->or_like('status', $get['s_status']);
            }
			//$this->db->group_end(); 
			$lead_manage_data = $this->db->get()->result_array();
			// print_r($this->db->last_query());
			
			/*$lead_manage_data = $this->db_model->select_data('student_lead_management.*,students.name as s_name,students.contact_no as contact','student_lead_management use index (id)','',$limit,array('id','desc'),$like,$join_array,'',$or_like);	*/
			if(!empty($lead_manage_data)){
                $role = $this->session->userdata('role');
                if($role == '1'){  
                    $profile = 'admin';
                }
    
                foreach($lead_manage_data as $lead_m){
				//print_r($lead_m);exit;
                 
                    if($lead_m['status'] == 1){
                        $statusDrop = '<div class="admin_tbl_status_wrap"><a class="tbl_status_btn light_sky_bg changeStatusButton"  data-id="'.$lead_m['id'].'" data-table =" student_lead_management" data-status ="0" href="javascript:;">'.$this->lang->line('ltr_active').'</a></div>';
                    }else{
                        $statusDrop = '<div class="admin_tbl_status_wrap">
                    <a class="tbl_status_btn light_red_bg changeStatusButton" data-id="'.$lead_m['id'].'" data-table =" student_lead_management" data-status ="1" href="javascript:;">'.$this->lang->line('ltr_inactive').'</a></div>';
                    }
					
					$action = '<p class="actions_wrap"><a href="add_lead_management/'.$lead_m['id'].'" class="edit_Category btn_edit" title="Edit" data-id="'.$lead_m['id'].'"><i class="fa fa-edit"></i></a>
					<a class="deleteData btn_delete" title="Delete" data-id="'.$lead_m['id'].'" data-table=" student_lead_management"><i class="fa fa-trash"></i></a></p>';
					 $dtl = '<input type="checkbox" class="checkOneRow" value="'.$lead_m['id'].'">';
					...
					$dataarray[] = array(
								...
								$lead_m['source'],
								$lead_m['enquiry_date'],
								$lead_m['enquiry_fromdate'],
								$lead_m['enquiry_todate'],
                                $statusDrop,
                                $action,
                                $added_by
                    ); 
                ...
                $recordsTotal = $this->db_model->countAll('student_lead_management use index (id)',$cond,'','',$like,'','',$or_like);
...
function subcategory_table(){
                ...
                foreach($question_data as $question){
                    ...
                    if($added_by){
                         if($added_by[0]['admin_id']==1 && $added_by[0]['super_admin'] == 1)
                         {
                             ...
                             $added_by = $added_by[0]['name']."  (Branch-Admin)";
                         }else{
 ...
 function add_video(){
	...
	function addEnquiry(){  
       if(isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')){
                ...
                if($id != ''){
					$id = $data_arr['id'];
					$data['name'] = $data_arr['name'];
					$data['mobile'] = $data_arr['mobile'];
					$data['message'] = $data_arr['message'];
                    $data['remarks'] = $data_arr['remarks'];
                    $data['enquiry_status'] = $data_arr['enquiry_status'];
                   
                    $update = $this->db_model->update_data_limit('enquiry',$data,array('id'=>$id));
					//print_r($this->db->last_query());exit;
					if($update==true){
                        $resp = array('status'=>1,'msg'=>$this->lang->line('ltr_enquiry_update_msg'));
                    }else{
                        $resp = array('status'=>0);
                    }
                    ...
                    $ins = $this->db_model->insert_data('enquiry',$data);
					//print_r($this->db->last_query());exit;

					if($ins==true){
                        $resp = array('status'=>1,'msg'=>$this->lang->line('ltr_enquiry_add_msg'));
                    }else{
                        $resp = array('status'=>0);
                    }
                }
        ...
        if(isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')){
            ...
            if(!empty($enquiry)){
                ...
                foreach($enquiry as $enq){
					...
					<a href="add-enquiry/'.$enq['id'].'" class="edit_Category btn_edit" title="Edit" data-id="'.$enq['id'].'"><i class="fa fa-edit"></i></a>
					<a class="deleteData btn_delete" title="Delete" data-id="'.$enq['id'].'" data-table="enquiry"><i class="fa fa-trash"></i></a></p>';
    ...
    //                      $added_by = $added_Data[0]['name']."  (Branch-Admin)";
    //                  }else{
	...
	function payment_history(){
        ...
        if(isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')){
            ...
            if($post['search']['value'] != ''){
                ...
                $like = array('name',$post['search']['value']);
                $or_like = $or_like = array(array('contact_no',$post['search']['value']),array('email',$post['search']['value']));
				$userdata = $this->db_model->select_data('id','students','','',array('id','desc'),$like,'','',$or_like);
				//echo $this->db->last_query();
				$usersId =array();
				foreach($userdata as $key){
					array_push($usersId,$key['id']);
...
//     }
    ...
    function get_teacher_subject(){
      ...
      public function techaer_edit_new(){
		...
		function addLeadManagement(){  
        /*if(isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')){
					$data_arr = $this->input->post(NULL,TRUE);
					if($this->input->post('type',TRUE) == 'edit'){
						$data = $data_arr['student_id'];
						$data['enquiry_date'] = date('Y-m-d',strtotime($data_arr['dob']));
					}
                    $data['student_id'] = $data_arr['student_id'];
                    $data['source'] = $data_arr['source'];
                    $data['enquiry_date'] = date('Y-m-d', strtotime($data_arr['enquiry_date']));
                    $data['enquiry_fromdate'] = date('Y-m-d', strtotime($data_arr['enquiry_fromdate']));
                    $data['enquiry_todate'] = date('Y-m-d', strtotime($data_arr['enquiry_todate']));
                    
                    $ins = $this->db_model->insert_data('student_lead_management',$data);

					if($ins==true){
                        $resp = array('status'=>1,'msg'=>$this->lang->line('ltr_leadmanage_add_msg'));
                    }else{
                        $resp = array('status'=>0);
                    }
                echo json_encode($resp,JSON_UNESCAPED_SLASHES);
		...
		if(isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')){
            //print_r('iii');exit();
				...
				$admin_id = $this->session->userdata('uid');
                if($id != ''){
					//print_r("addlead1");exit;
                    $id = $data_arr['id'];
					$data['name'] = $data_arr['name'];
					$data['contact'] = $data_arr['contact'];
                    //$data['student_id'] = $data_arr['student_id'];
                    $data['source'] = $data_arr['source'];
                    $data['enquiry_date'] = date('Y-m-d', strtotime($data_arr['enquiry_date']));
                    $data['enquiry_fromdate'] = date('Y-m-d', strtotime($data_arr['enquiry_fromdate']));
                    ...
                    $update = $this->db_model->update_data_limit('student_lead_management',$data,array('id'=>$id));
					//print_r($this->db->last_query());exit;
					if($update==true){
                        $resp = array('status'=>1,'msg'=>$this->lang->line('ltr_leadmanage_update_msg'));
                    }else{
                        $resp = array('status'=>0);
                    }
	...
	function addVisitors(){  
		if(isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')){
                ...
                if($id != ''){
					//print_r("if");exit;
					$id = $data_arr['id'];
					$data['purpose'] = $data_arr['purpose'];
					$data['v_name'] = $data_arr['v_name'];
					$data['v_contact'] = $data_arr['v_contact'];
					$data['id_card'] = $data_arr['id_card'];
					$data['num_of_person'] = $data_arr['num_of_person'];
                    $data['date'] = date('Y-m-d', strtotime($data_arr['date']));
                    $data['in_time'] = $data_arr['in_time'];
                    $data['out_time'] = $data_arr['out_time'];
                 
                    $update = $this->db_model->update_data_limit('visitors',$data,array('id'=>$id));
					//print_r($this->db->last_query());exit;
					if($update==true){
                        $resp = array('status'=>1,'msg'=>$this->lang->line('ltr_visitors_update_msg'));
                    }else{
                        $resp = array('status'=>0);
                    }
					...
					$this->form_validation->set_rules('v_contact', 'Contact Number ', 'required|regex_match[/^[0-9]{10}$/]');
					 if ($this->form_validation->run() == FALSE) { 
						...
						$ins = $this->db_model->insert_data('visitors',$data);
						//print_r($this->db->last_query());exit;
					...
					if($ins==true){
                        $resp = array('status'=>1,'msg'=>$this->lang->line('ltr_visitors_add_msg'));
                    }else{
                        $resp = array('status'=>0);
                    }
                }
	...
	function visitors_table(){
        if(isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')){
            ...
            if($post['search']['value'] != ''){
                $like = array('v_name',$post['search']['value']);
                $or_like = array(array('v_contact',$post['search']['value']));
            }else{
               $like = ''; 
               $or_like = ''; 
            }
    
            $visitors = $this->db_model->select_data('*','visitors use index (id)','',$limit,array('id','desc'),$like,'','',$or_like);
            ...
            if(!empty($visitors)){
                ...
                foreach($visitors as $vst){
                    //$messageWord =$this->readMoreWord($enq['message'], 'Message');
                    //$action = '<p class="actions_wrap"><a class="deleteData btn_delete" title="Delete" data-id="'.$vst['id'].'" data-table="visitors"><i class="fa fa-trash"></i></a></p>';
					
					$action = '<p class="actions_wrap">
					<a href="'.base_url().'admin/visitors/'.$vst['id'].'" class="edit_Category btn_edit" title="Edit" data-id="'.$vst['id'].'"><i class="fa fa-edit"></i></a>
					<a class="deleteData btn_delete" title="Delete" data-id="'.$vst['id'].'" data-table="visitors"><i class="fa fa-trash"></i></a>
					</p>';
					$dtl = '<input type="checkbox" class="checkOneRow" value="'.$vst['id'].'">';
                ...
                $recordsTotal = $this->db_model->countAll('visitors use index (id)','','','',$like,'','',$or_like);
	...
	function placement_tracker(){
		//print_r("hgjkdhgdf");exit;
        if(isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')){
            $post = $this->input->post(NULL,TRUE);
            $get = $this->input->get(NULL,TRUE);
            if(isset($post['length']) && $post['length']>0){
            ...
            if($post['search']['value'] != ''){
                $like = array('name',$post['search']['value']);
                $or_like = array(array('contact_no',$post['search']['value']));
                $search="AND((st.name LIKE '%".$post['search']['value']."%') OR ( st.contact_no LIKE '%".$post['search']['value']."%'))";
            }else{
               $like = ''; 
               $or_like = ''; 
            }
    
            //$student = $this->db_model->select_data('*','students use index (id)','',$limit,array('id','desc'),$like,'','',$or_like);
			
			//select st.id,st.name as student_name, st.contact_no, st.enrollment_id as student_ref_no, b.end_date, sb.batch_id, st.name, b.cat_id, b_cat.name as category, p.company, p.student_id, sub_cat.name as subcategory_name, b.batch_price as course_fees, payment.amount as amount_paid from students st left join sudent_batchs sb on st.id = sb.student_id left join batches b on sb.batch_id = b.id left join batch_category b_cat on b.cat_id = b_cat.id left join placement p on st.id = p.student_id left join batch_subcategory sub_cat on b_cat.id = sub_cat.cat_id left join student_payment_history payment on st.id = payment.student_id where sb.batch_id != 0
			
            //$search="AND st.name LIKE 'Sajin' OR st.contact_no LIKE '0987654321'";

             

			$query = $this->db_model->sql_query("select st.id,st.name as student_name, st.contact_no, st.enrollment_id as student_ref_no, b.end_date, sb.batch_id, st.name, b.cat_id, b_cat.name as category, p.company, p.student_id, sub_cat.name as subcategory_name, b.batch_price as course_fees, payment.amount as amount_paid from students st left join sudent_batchs sb on st.id = sb.student_id left join batches b on sb.batch_id = b.id left join batch_category b_cat on b.cat_id = b_cat.id left join placement p on st.id = p.student_id left join batch_subcategory sub_cat on b_cat.id = sub_cat.cat_id left join student_payment_history payment on st.id = payment.student_id where sb.batch_id != 0 $search ");
           ...
           // print_r($this->db->last_query());exit;
            ...
            if(!empty($student)){
				...
				//print_r($st);
					...
					$current_date = date("Y-m-d");
					if($st['batch_id'] == ''){
						$student_status = '<a class="btn btn-danger" title="Edit" data-id="'.$st['id'].'">'.$this->lang->line('ltr_inactive').'</i></a>';
					}elseif($st['end_date'] < $current_date && $st['company']==''){
						$student_status = '<a class="btn btn-success" title="Edit" data-id="'.$st['id'].'">'.$this->lang->line('ltr_active').'</i></a>';
					}elseif($st['end_date'] > $current_date && $st['company']==''){
						$student_status = '<a class="btn btn-info" title="Edit" data-id="'.$st['id'].'">'.$this->lang->line('ltr_passedout').'</i></a>';
					}elseif($st['company']!=''){
						$student_status = $st['company'];
					}else{
						$student_status = '<div></div>';
					}
					...
					if($st['batch_id'] == ''){
						...
						$action = '<a class="btn btn-success addplacement" title="Edit" data-id="'.$st['id'].'">'.$this->lang->line('ltr_graduate').'</i></a>';
						//$dtl = '<input type="checkbox" class="checkOneRow" value="'.$st['id'].'">';
					}elseif($st['company']!=''){
						$action = $st['company'];
					}else{
						$action = '<div></div>';
					...
					if($st['course_fees'] != ''){
						$amount_pending = $st['course_fees'] - $st['amount_paid'];
					}else{
						$amount_pending = '0';
					}
                ...
                $recordsTotal = $this->db_model->countAll('students use index (id)','','','',$like,'','',$or_like);
	...
	function addPlacementTracker(){  
		if(isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')){
            
                $data_arr = $this->input->post(NULL,TRUE);
				$id = $data_arr['id'];
				$stud_id = $data_arr['student_id'];
				//print_r($data_arr);
				$admin_id = $this->session->userdata('uid');
				
				$query = $this->db_model->sql_query('SELECT * FROM `placement` WHERE `student_id` = '.$stud_id);
				//print_r($this->db->last_query());exit;
				$find_studentId = $query->result_array(); 
				
                if($find_studentId == ''){
					//print_r("if");exit;
                    $id = $data_arr['student_id'];
                    $data['placement_date'] = date('Y-m-d', strtotime($data_arr['placement_date']));
					$data['company'] = $data_arr['company'];
                    //$data['student_id'] = $data_arr['student_id'];
                    $data['position'] = $data_arr['position'];
					$data['salary'] = $data_arr['salary'];
					$data['contract_month'] = $data_arr['contract_month'];
					$data['description'] = $data_arr['description'];
                    
                    $update = $this->db_model->update_data_limit('placement',$data,array('placement.student_id'=>$id));
					//print_r($this->db->last_query());exit;
					if($update==true){
                        $resp = array('status'=>1,'msg'=>$this->lang->line('ltr_placement_update_msg'));
                    }else{
                        $resp = array('status'=>0);
                    }
					...
					//print_r("else");exit;
					$data_arror = $this->input->post(NULL,TRUE);
					if($this->input->post('type',TRUE) == 'edit'){
						$data = $data_arror['student_id'];
					}
					$data['student_id'] = $data_arror['student_id'];
					$data['placement_date'] = date('Y-m-d', strtotime($data_arr['placement_date']));
					$data['company'] = $data_arror['company'];
                    $data['position'] = $data_arror['position'];
					$data['salary'] = $data_arror['salary'];
					$data['contract_month'] = $data_arror['contract_month'];
					$data['description'] = $data_arror['description'];
                    
                    $ins = $this->db_model->insert_data('placement',$data);

					if($ins==true){
                        $resp = array('status'=>1,'msg'=>$this->lang->line('ltr_placement_add_msg'));
                    }else{
                        $resp = array('status'=>0);
                    }
                }
	...
	function student_placement_table($id){
        if(isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')){
            $post = $this->input->post(NULL,TRUE);
            $get = $this->input->get(NULL,TRUE);
            if(isset($post['length']) && $post['length']>0){
            ...
            $student_placement = $this->db_model->select_data('*','placement use index (id)','',$limit,array('id','desc'),$like,'','',$or_like);
    
            if(!empty($student_placement)){
                ...
                foreach($student_placement as $sp){
                    //$messageWord =$this->readMoreWord($enq['message'], 'Message');
                    //$action = '<p class="actions_wrap"><a class="deleteData btn_delete" title="Delete" data-id="'.$sp['id'].'" data-table="visitors"><i class="fa fa-trash"></i></a></p>';
					
					$action = '<p class="actions_wrap">
					<a href="'.base_url().'admin/visitors/'.$sp['id'].'" class="edit_Category btn_edit" title="Edit" data-id="'.$sp['id'].'"><i class="fa fa-edit"></i></a>
					<a class="deleteData btn_delete" title="Delete" data-id="'.$sp['id'].'" data-table="visitors"><i class="fa fa-trash"></i></a>
					</p>';
					//$dtl = '<input type="checkbox" class="checkOneRow" value="'.$sp['id'].'">';
                ...
                $recordsTotal = $this->db_model->countAll('placement use index (id)','','','',$like,'','',$or_like);
	...
	function addCallLog(){  
		if(isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')){
                ...
                if($id != ''){
					...
					$id = $data_arr['id'];
					$data['name'] = $data_arr['name'];
					$data['phone'] = $data_arr['phone'];
					$data['date'] = date('Y-m-d', strtotime($data_arr['date']));
					$data['description'] = $data_arr['description'];
					$data['next_followup_date'] = date('Y-m-d', strtotime($data_arr['next_followup_date']));
                    $data['call_duration'] = $data_arr['call_duration'];
					$data['note'] = $data_arr['note'];
                 
                    $update = $this->db_model->update_data_limit('call_log',$data,array('id'=>$id));
					//print_r($this->db->last_query());exit;
					if($update==true){
                        $resp = array('status'=>1,'msg'=>$this->lang->line('ltr_call_log_update_msg'));
                    }else{
                        $resp = array('status'=>0);
                    }
					...
					$this->form_validation->set_rules('phone', 'Phone Number ', 'required|regex_match[/^[0-9]{10}$/]');
					 if ($this->form_validation->run() == FALSE) { 
						...
						$data['next_followup_date'] = date('Y-m-d', strtotime($data_arr['next_followup_date']));
						$data['call_duration'] = $data_arr['call_duration'];
						$data['call_type'] = $data_arr['call_type'];
						$data['note'] = $data_arr['note'];
						
						$ins = $this->db_model->insert_data('call_log',$data);
						
					 } 
					

					if($ins==true){
                        $resp = array('status'=>1,'msg'=>$this->lang->line('ltr_call_log_add_msg'));
                    }else{
                        $resp = array('status'=>0);
                    }
                }
	...
	function call_log_table(){
		if(isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')){
			...
			if($post['search']['value'] != ''){
                $like = array('name',$post['search']['value']);
                $or_like = array(array('phone',$post['search']['value']));
            }else{
               $like = ''; 
               $or_like = ''; 
            }
    
            $call_log = $this->db_model->select_data('*','call_log use index (id)','',$limit,array('id','desc'),$like,'','',$or_like);
            ...
            if(!empty($call_log)){
                ...
                foreach($call_log as $clg){
					
					$action = '<p class="actions_wrap">
					<a href="'.base_url().'admin/call_log/'.$clg['id'].'" class="edit_Category btn_edit" title="Edit" data-id="'.$clg['id'].'"><i class="fa fa-edit"></i></a>
					<a class="deleteData btn_delete" title="Delete" data-id="'.$clg['id'].'" data-table="call_log"><i class="fa fa-trash"></i></a>
					</p>';
					$dtl = '<input type="checkbox" class="checkOneRow" value="'.$clg['id'].'">';
                ...
                $recordsTotal = $this->db_model->countAll('call_log use index (id)','','','',$like,'','',$or_like);
