			...
			if(empty($prevRecd)){
			   ...
			   $title = $this->db_model->select_data('site_title','site_details','',1,array('id','desc'))[0]['site_title'];
				$subj = $title.'- '.$this->lang->line('ltr_thanks_msg');
					$em_msg = $this->lang->line('ltr_hey').' '.ucwords($name).', '.$this->lang->line('ltr_congratulations').' <br/><br/>'.$this->lang->line('ltr_successfully_enrolled_in').' '.$batch_type[0]['batch_name'].'<br/><br/>'.$this->lang->line('ltr_enjoy').'<br/><br/> '.$this->lang->line('ltr_thanks_msg').'<br/><br/>'.$this->lang->line('ltr_bizacademy');
				@$this->SendMail($email, $subj, $em_msg);
