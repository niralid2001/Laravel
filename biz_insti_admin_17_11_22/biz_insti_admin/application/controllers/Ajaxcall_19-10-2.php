...
function category_table(){
    ...
    function lead_management_table(){
        ...
        if(isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')){
			...
			if($post['search']['value'] != ''){
                $join_array = array('students',"students.name like '%".$post['search']['value']."%' AND students.id = student_lead_management.student_id");
            }else{
              $join_array = array('students',"students.id = student_lead_management.student_id");
            }
			...
			$this->db->select('student_lead_management.*,students.name as s_name,students.contact_no as contact');
			...
			//$this->db->group_start(); 
			if($get['s_source'] != ''){
				$this->db->like('student_lead_management.source' , $get['s_source']);
            }
			
			if($get['s_fromdate'] != ''){
			$s_fromdate = date('Y-m-d', strtotime($get['s_fromdate']));
               $this->db->or_like('student_lead_management.enquiry_fromdate', $s_fromdate);
            }
			
			if($get['s_todate'] != ''){
			$s_todate = date('Y-m-d', strtotime($get['s_todate']));
                $this->db->or_like('student_lead_management.enquiry_todate',  $s_todate);
            }
			
			if($get['s_status'] != ''){
                $this->db->or_like('student_lead_management.status', $get['s_status']);
            }
			//$this->db->group_end(); 
