...
function category_table(){
    ...
    function lead_management_table(){
        ...
        if(isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')){
			...
			if($get['s_source'] != ''){
				$like = array('student_lead_management.source' => '"'.$get['s_source'].'"');
            }
			
			if($get['s_fromdate'] != ''){
			$s_fromdate = date('Y-m-d', strtotime($get['s_fromdate']));
                $like['student_lead_management.enquiry_fromdate'] = $s_fromdate;
            }
			
			if($get['s_todate'] != ''){
			$s_todate = date('Y-m-d', strtotime($get['s_todate']));
                $like['student_lead_management.enquiry_todate'] = $s_todate;
            }
			
			if($get['s_status'] != ''){
                $like['student_lead_management.status'] = $get['s_status'];
            }
