...
<div class="headingtraslate">
    <div class="container mb-5">
        <div class="row ">
            <div class="col-12 col-sm-6 col-md-5 col-lg-4">
                @if($brokers->count())
                    ...
                    @foreach($brokers as $bdetail)
                        <div class=" row company-side-card mb-3">

                            <div class="col-4"> 
                                  <a href='https://{{$bdetail->broker_url}}' target="_blank" class="dtradebtn"  type="button" >{{__("TradeNow")}}</a>
                                    <a href="{{url($bdetail->slug.'-'.$bdetail->id)}}" target="_blank"  class="dbrokerbtn"  type="button" >
                                    {{__("Broker Reviews")}}</a> 
                            </div>
                            ...
                            <div class="col-3">
                                <div class="minimumimg" >
                                    <div class="minimumimgposition"> 
                                        <img class="img-fluid h-25 " src="{{url('uploads/broker/'.$bdetail->image)}}" >
                                    </div>
                                </div>
                            </div>
                ...
                <div class="col-12 row trade-form-side-card">
                    <form method="post" name="trading-form" data-parsley-validate action="{{route('front_save-trade-user')}}">
                        ...
                        <button type="submit" class="btn " style='  background:linear-gradient(90deg, rgba(69,161,68,1) 37%, rgba(141,191,69,1) 54%);  color:white; ' >{{__("Submit")}}</button>
                    </form>
                </div>
            ...
            <div class="col-12 col-sm-6 col-md-7 col-lg-8 mt-3">
                <h3 >{{isset($detail->title) ? $detail->title : ''}}</h3>
                <img class="img-fluid" src="{{url('uploads/artical/'.$detail->image)}}" alt="">
                <p>
