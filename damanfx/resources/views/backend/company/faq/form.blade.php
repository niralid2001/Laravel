    ...
    <div class="container-fluid p-0">
        ...
        <div class="row">
            <div class="col-md-12">
                <div class="card">
                    <div class="card-body">
                        <form method="POST" action="@if(isset($record)){{ route('admin_company-faq-update', array('id' => Crypt::encrypt($record->id))) }}@else{{route('admin_company-faq-store')}}@endif" id="news-form" enctype="multipart/form-data" data-parsley-validate="">
                            ...
                            <a href="@if(isset($record)) {{route('admin_company-faqs', array($record->company_id))}} @else {{route('admin_company-faqs', array($company_id))}} @endif" class="btn btn-danger">{{__('Cancel')}}</a>
                        </form>
                    </div>
                </div>
            </div>
